...
10. **Point the Analyst to the next step.** The brief you just wrote is saved into the working folder but is **not yet on the shared `dev` branch** — this skill does not commit. Tell the Analyst how to proceed: _"Your requirements brief is complete. Run `/plan` to start designing the app — it will first publish your brief to the shared `dev` branch (after a quick secret/PII safety scan), then plan from it."_ Do **not** run any git commands here, and the Analyst does **not** need a separate publish command — `/plan`'s first step publishes the brief for them.
